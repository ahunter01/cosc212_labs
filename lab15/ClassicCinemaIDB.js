var ClassicCinemaIDB = (function () {
    ...
    */
    ...
    request.onupgradeneeded = function (event) {
        ...
        // and the same for the reviews object store
        reviewObjectStore.transaction.addEventListener('complete',function (event) {
            console.log("adding movie data");
            // Store values in the newly created objectStore.
            var customerObjectStoreTransaction =  db.transaction("reviews", "readwrite").objectStore("reviews");
    ...
    /**
     * Displays all the reviews in the Object store
     */
    pub.displayAllReviews = function(){
        var objectStore = getObjectStore("reviews","readonly");

        objectStore.openCursor().onsuccess = function(event) {
            var cursor = event.target.result;
            if (cursor) {
                console.log("Title " + cursor.value.title + ". Review: " + cursor.value.review + " Review By: " + cursor.value.user);
                cursor.continue();
            }
            else {
